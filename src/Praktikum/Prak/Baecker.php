...
EOT;
    ...
    protected function getViewData():array{
        ...
        $sqlselect = "Select * from ordered_article where status='0' or status='1' or status='2'";
        $result = $this->database->query($sqlselect);
        $i = 0;
        if ($result->num_rows > 0) {
            // output data of each row
            while($row = $result->fetch_assoc()) {
                $ordered_article[$i][0] = $row["ordered_article_id"];
                $ordered_article[$i][1] = $row["ordering_id"];
                $ordered_article[$i][2] = $row["article_id"];
                $ordered_article[$i][3] = $row["status"];
